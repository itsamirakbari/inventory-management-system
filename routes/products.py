...
def products():
    try:
        ...
        logger.exception(f"Database error while loading products | User: {session['user']['username']}")
        return redirect(url_for("dashboard.dashboard"))

    except Exception:
        flash("An unexpected error occurred.", "error")
        logger.exception(f"Unexpected error while loading products | User: {session['user']['username']}")
        return redirect(url_for("dashboard.dashboard"))
...
def add_product_route():
    if request.method == "GET":
        try:
            ...
            flash("Database error. Please try again.", "error")
            logger.exception(f"Database error while loading the add product form | User: {session['user']['username']}")
            return redirect(url_for("products.products"))

        except Exception:
            flash("An unexpected error occurred.", "error")
            logger.exception(f"Unexpected error while loading the add product form | User: {session['user']['username']}")
            return redirect(url_for("products.products"))
    ...
    try:
        ...
        flash("Invalid price. Please check your inputs.", "error")
        return redirect(url_for("products.add_product_route"))
...
def delete_product_route(product_id):
    try:
        ...
        flash("Database error. Please try again.", "error")
        logger.exception(f"Database error while deleting product | Product ID: {product_id} | User: {session['user']['username']}")
        return redirect(url_for("products.products"))

    except Exception:
        flash("An unexpected error occurred.", "error")
        logger.exception(f"Unexpected error while deleting product | Product ID: {product_id} | User: {session['user']['username']}")
        return redirect(url_for("products.products"))
...
def update_product_route(product_id):
    ...
    try:
        ...
        flash("Database error. Please try again.", "error")
        logger.exception(f"Database error while updating product | Product ID: {product_id} | New Product: '{product_name}' | New SKU: '{product_code}' | User: {session['user']['username']}")
        return redirect(url_for("products.products"))

    except Exception:
        flash("An unexpected error occurred.", "error")
        logger.exception(f"Unexpected error while updating product | Product ID: {product_id} | New Product: '{product_name}' | New SKU: '{product_code}' | User: {session['user']['username']}")
        return redirect(url_for("products.products"))
...
def update_product_status_route(product_id):
    try:
        ...
        flash("Database error. Please try again.", "error")
        logger.exception(f"Database error while updating product status | Product ID: {product_id} | User: {session['user']['username']}")
        return redirect(url_for("products.products"))

    except Exception:
        flash("An unexpected error occurred.", "error")
        logger.exception(f"Unexpected error while updating product status | Product ID: {product_id} | User: {session['user']['username']}")
        return redirect(url_for("products.products"))
